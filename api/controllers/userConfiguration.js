...
module.exports={
    ...
    {
        mySqlConnection.query(`SELECT a.*, TIMESTAMPDIFF(YEAR, a.date_of_birth, CURDATE()) AS age FROM user_configuration a`, (err,rows) =>
        {
            if(!err)
            {
                return cb(rows);
    ...
    createUserConfiguration: (req,res)=>{
        ...
        /////////////
        mySqlConnection.query(`SELECT user_id from users where email=?`, [req.body.email], (err, rows)=>
        {
            if (err)
            {
                console.log(err);
            ...
            else
            {
                ...
                mySqlConnection.query(`INSERT INTO user_configuration (user_id, first_name, last_name, date_of_birth, city, gender, phone_number, registration_date, relationship_status, sexual_orientation, profession, pronoun, hobbies, radius, longitude, latitude) VALUES ("${user_id}","${first_name}","${last_name}","${dateOfBirth}","${city}","${gender}","${phoneNumber}","${registerDate}","${relationship_status}","${sexual_orientation}","${profession}","${pronoun}", "${hobbies}" ,"${radius}","${longitude}","${latitude}")`,(err,result)=> {
                    if(!err)
                    {
    ...
    updateUserConfiguration: (req,res) => {
        ...
        mySqlConnection.query("UPDATE user_configuration SET first_name=?, last_name=?, password=?, dateOfBirth=?, city=?, gender=?, phone_number=?, registration_date=?, relationship_status=?, sexual_orientation=?, profession=?, pronoun=?, hobbies=? ,radius=?, longitude=?, latitude=? WHERE user_id=?", [first_name, last_name,  dateOfBirth, city, gender, phoneNumber, registerDate, relationship_status,sexual_orientation,profession,pronoun, hobbies ,radius,longitude,latitude,req.params.user_id], (err,result)=> {
            if(!err)
            {
